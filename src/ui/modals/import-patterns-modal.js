...
exports.ImportPatternsModal = void 0;
const obsidian_1 = require("obsidian");
const patterns_1 = require("../../constants/patterns");
class ImportPatternsModal extends obsidian_1.Modal {
    ...
    onOpen() {
        ...
        previewContent.style.cssText = patterns_1.CSS_STYLES.PREVIEW_CONTENT;
        ...
        importButton.onclick = () => {
            ...
            if (!file) {
                new obsidian_1.Notice('Please select a file');
                return;
            }
            const reader = new FileReader();
            reader.onload = (e) => {
                var _a;
                const content = (_a = e.target) === null || _a === void 0 ? void 0 : _a.result;
                const mergeMode = mergeRadio.checked;
                const result = this.plugin.importPatternsFromJson(content, mergeMode);
                if (result.success) {
                    new obsidian_1.Notice(`Successfully imported ${result.imported} patterns`);
                    this.settingsTab.display();
                    this.close();
                }
                else {
                    new obsidian_1.Notice(`Import failed: ${result.error}`);
                }
            };
            reader.readAsText(file);
        };
...
exports.ImportPatternsModal = ImportPatternsModal;
